...
<form action="{{route('save-pdf')}}" method="post" enctype="multipart/form-data">  
   ...
   <div class="form-group col-md-6 mt-5 offset-3">
    ...
    <div class="mt-5">
        ...
        <input type="file" value="{{old('readUrl')}}" class="form-control mt-2 @error('readUrl') is-invalid @enderror" name="readUrl">
        @error('readUrl')
               <div class="alert alert-danger">{{$message}}</div> 
            @enderror
        <div class="form-floating mb-1 mt-2">
          <input type="text" value="{{old('ageGroup')}}" class="form-control @error('ageGroup') is-invalid @enderror" name="ageGroup" placeholder="Age">
          @error('ageGroup')
               <div class="alert alert-danger">{{$message}}</div> 
            @enderror
